...
export function extractBracketPairs(input) {
  const regex = /\(([^)]*)\)|[^(]+/g; // Match text inside parentheses or non-parenthesis text
  const matches = input.match(regex)?.map(str => str.replace(/^\(|\)$/g, '').trim()) || [];
  return matches;
}